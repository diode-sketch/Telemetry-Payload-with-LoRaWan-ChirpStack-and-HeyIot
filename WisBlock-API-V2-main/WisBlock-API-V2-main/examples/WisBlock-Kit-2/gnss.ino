...
bool poll_gnss(void)
{
	...
	Serial1.begin(9600);
	delay(500);
	if (my_GNSS.begin(Serial1))
	{
		if (my_GNSS.setDynamicModel(DYN_MODEL_AIRBORNE_1G))
		{
			MYLOG("GNSS", "Airborne mode set!");
			my_GNSS.saveConfiguration()
		}
		else
		{
			MYLOG("GNSS","Failed to set Airborne mode")
		}
	}
	else
	{
		MYLOG("GNSS","Failed to begin the u-blox gnss")
	}
